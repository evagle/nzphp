...
class SwooleRpcClient
{
    ...
    protected function getConnection($host, $port)
    {
        ...
        if ($this->haveSwoole) {
            ...
            $socket->set(array(
                ...
                'package_body_offset' => SwooleRpcServer::HEADER_SIZE,
    ...
    protected function request($send, $retObj)
    {
        ...
        //打包格式
        $encodeType = $this->encode_json ? SwooleRpcServer::DECODE_JSON : SwooleRpcServer::DECODE_PHP;
        if ($this->encode_gzip) {
            $encodeType |= SwooleRpcServer::DECODE_GZIP;
        }
        //发送失败了
        if ($retObj->socket->send(SwooleRpcServer::encode($retObj->send, $encodeType, 0, $retObj->requestId)) === false) {
            //连接被重置了，重现连接到服务器
    ...
    protected function recvPacket($connection)
    {
         ...
         * Stream or Socket
         */
        $_header_data = $connection->recv(SwooleRpcServer::HEADER_SIZE, true);
        if (empty($_header_data)) {
            return "";
        }
        //这里仅使用了length和type，uid,serid未使用
        $header = unpack(SwooleRpcServer::HEADER_STRUCT, $_header_data);
        //错误的包头，返回空字符串，结束连接
        ...
        while (count($this->waitList) > 0) {
            ...
            if ($n > 0) {
                ...
                foreach ($read as $connection) {
                    ...
                    $header = unpack(SwooleRpcServer::HEADER_STRUCT, substr($data, 0, SwooleRpcServer::HEADER_SIZE));
                    //不在请求列表中，错误的请求串号
                    ...
                    //成功处理
                    $this->finish(SwooleRpcServer::decode(substr($data, SwooleRpcServer::HEADER_SIZE), $header['type']), $retObj);
                    $success_num++;
                }
            }
            //发生超时
